...
from seq2seq_translation.seq2seq_cnn import Seq2SeqCNN
...
src_seqs = df['pred sentence']
tgt_seqs = df['sentence']
...
net = Seq2SeqCNN(
    in_channels=len(corpus),
    out_channels=len(corpus),
    kernel_size=5,
    n_layers=8)
...
while True:
    ...
    srcs = torch.cat(srcs, 0).float().to(device).permute(0, 2, 1)
    tgts = torch.cat(tgts, 0).float().to(device)
    
    optimizer.zero_grad()

    out = net(srcs)
    cost = loss(out, tgts.argmax(2)) * alpha

    cost.backward()
    optimizer.step()

    trn_cost += cost.detach().cpu().item() / batch_size
    ...
    if (c > 0) and (c % log_step == 0):
        ...
        for ix in range(n):
            ...
            srcs = torch.cat([torch.Tensor(src)], 0).float().to(device).permute(0, 2, 1)
            ...
            tgt = tgts[0:1].permute(0, 2, 1).detach().cpu()[0]
        ...
        torch.save(state_dict, os.path.join(dir_output, 'seq2seq-lstm-state.pt'))
